...
router.route("/").get(verifyToken, getAllCalls).post(verifyToken, createCallValidation, createCall);
router.route("/:callId").get(verifyToken, callIdValidation, getCallById).patch(verifyToken, callUpdateValidation, updateCall).delete(verifyToken, callIdValidation, deleteCall);
