...
export const createTransaction = async (req, res) => {
    try {
        const {callId, engineerName, itemName, modelNumber, serialNumber, stockId, category} = req.body;
        ...
        return res.status(error.statusCode || 500).json(new ApiResponse(error.statusCode, null, error.message || "Something went wrong while creating transaction"));
    }
};
...
export const getTransactions = async (req, res) => {
    try {
        ...
        return res.status(error.statusCode || 500).json(new ApiResponse(error.statusCode, null, error.message || "Something went wrong while retrieving transactions"));
    }
};
...
export const getTransaction = async (req, res) => {
    try {
        ...
        return res.status(error.statusCode || 500).json(new ApiResponse(error.statusCode, null, error.message || "Something went wrong while retrieving transaction"));
    }
};