...
const createFilter = (query) => {
    ...
    if (query.employeeDesignation) {
        filter.employeeDesignation = {
            $regex: query.employeeDesignation,
            $options: "i",
        };
    }
    if (query.status) {
    ...
    if (query.resignedAt) {
        filter.resignedAt = {
            ...
            $lt: new Date(
                new Date(query.resignedAt).setUTCHours(23, 59, 59, 999)
            ),
        };
    }
    ...
    if (query.joinDate) {
        filter.joinDate = {
            ...
            $lt: new Date(
                new Date(query.joinDate).setUTCHours(23, 59, 59, 999)
            ),
        };
    }
    if (query.employeeDOB) {
        filter.employeeDOB = {
            ...
            $lt: new Date(
                new Date(query.employeeDOB).setUTCHours(23, 59, 59, 999)
            ),
        };
    }
    return { filter, sort };
...
export const createEngineer = async (req, res) => {
    try {
        ...
        if (employeeDOB > joinDate) {
            throw new ApiError(
                400,
                "Employee DOB cannot be greater than Join date"
            );
        }
        ...
        return res
            .status(200)
            .json(
                new ApiResponse(201, engineer, "Engineer created successfully")
            );
    } catch (error) {
        return res
    ...
    try {
        ...
        if (resignedAt) {
            if (engineer.joinDate > resignedAt) {
                throw new ApiError(
                    400,
                    "Engineer cannot be resigned before joining. Please check join date and resigned date"
                );
            }
        ...
        return res
            .status(200)
            .json(
                new ApiResponse(200, engineer, "Engineer updated successfully")
            );
    } catch (error) {
        return res
            .status(error.statusCode || 500)
